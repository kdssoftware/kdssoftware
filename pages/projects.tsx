    ...
    return (<div className="grid w-full grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {
            projects.map(project => (
                <div className="relative grid grid-flow-row grid-rows-1 p-3 rounded-lg bg-yellow-800 opacity-90 auto-rows-max" key={project.title}>
                    ...
                    <div className="flex flex-wrap justify-center py-2 text-lg">
                        {project.technologies.map(tech => (
                            <span className="px-2 mx-1 mt-1 rounded-full bg-yellow-900" key={tech}>{tech}</span>
                        ))}
                    </div>
