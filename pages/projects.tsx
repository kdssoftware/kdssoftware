...
const Projects: NextPage = () => {
  return (
    <div className="grid mx-10 py-14 grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 fixed top-14 md:top-32  mt-12 left-0 overflow-y-auto bottom-1 md:bottom-12">
      {projects.map((project) => (
        <div
          className="relative grid grid-flow-row grid-rows-3 p-3 rounded-lg bg-yellow-600 opacity-90 "
          key={project.title}
        >
          ...
          <div className="flex flex-wrap justify-center py-2 text-lg">
            {project.technologies.map((tech) => (
              <span
                className="px-2 mx-1 mt-1 rounded-full max-h-7 bg-yellow-700"
                key={tech}
              >
                {tech}
              </span>
