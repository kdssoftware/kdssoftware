...
const Navbar : NextPage<Props> = (props) =>{
    ...
    <header className="w-full absolute left-0 top-0 p-6 md:p-12 lg:p-16">
        ...
        <ul className="flex">
        {
            ...
            <li key={index} className={"ml-12 md:ml-24 "+(hoveringNavNumber===index || currentIndex === index?"text-yellow-200":"")} >
                <Link href={item.href}>
                  <a className='cursor-pointer' onMouseOver={()=> {animateNavBorder(index)}} onMouseLeave={()=> {animateNavBorder(-1)}}>
                      <div className="flex items-center justify-end">
                      <div className={"w-10 border-b border-solid border-white ease-in-out  transition  "+(currentIndex===index?"border-4 border-yellow-500 border-10 rounded-lg antialiased ":hoveringNavNumber===index?"border-4 border-yellow-500 border-10 rounded-lg animate-pulse antialiased":"")}></div>
